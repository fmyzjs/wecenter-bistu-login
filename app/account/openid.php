...
class openid extends AWS_CONTROLLER
{
	...
	public function bistu_action()
	{
		...
		unset(AWS_APP::session()->bistu_profile);
		unset(AWS_APP::session()->bistu_token);
		
		$oauth = new Services_Bistu_BistuOAuth(get_setting('bistu_akey'), get_setting('bistu_skey'));
		
		HTTP::redirect($oauth->getAuthorizeURL(get_js_url('/account/openid/bistu_callback/')));
	}

	public function bistu_callback_action()
	{
		if (get_setting('bistu_oauth_enabled') != 'Y')
		{
			HTTP::redirect('/account/login/');
		...
		if ($this->is_post() and AWS_APP::session()->bistu_profile and ! AWS_APP::session()->bistu_token['error'])
		{
			if (get_setting('register_type') == 'close')
			{
				H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('本站目前关闭注册')));
			...
			if ($uid)
			{
				$this->model('openid_bistu')->bind_account(AWS_APP::session()->bistu_profile, null, $uid, true);
				if (AWS_APP::session()->bistu_profile['profile_image_url'])
				{
					$this->model('account')->associate_remote_avatar($uid, str_replace('/50/', '/180/', AWS_APP::session()->bistu_profile['profile_image_url']));
				}
			...
			{
				H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('与 Bistu 通信出错 (Register), 请重新登录')));
			}
		}
		else
		{
			if ($_GET['code'] and (! AWS_APP::session()->bistu_token or AWS_APP::session()->bistu_token['error']))
			{
				$oauth = new Services_Bistu_BistuOAuth(get_setting('bistu_akey'), get_setting('bistu_skey'));
				
				AWS_APP::session()->bistu_token = $oauth->getAccessToken('code', array(
					'code' => $_GET['code'], 
					'redirect_uri' => get_js_url('/account/openid/bistu_callback/')
				));
				...
				//$uid_get = $client->get_uid();
				//$bistu_profile = $client->show_user_by_id($uid_get['uid']);
			...
			{
				H::redirect_msg(AWS_APP::lang()->_t('与 Bistu 通信出错, 请重新登录'), '/account/login/');
			}
			
			if ($bistu_user = $this->model('openid_bistu')->get_users_bistu_by_id(AWS_APP::session()->bistu_profile['userid']))
			{
				$user_info = $this->model('account')->get_user_info_by_uid($bistu_user['uid']);
				
				HTTP::set_cookie('_user_login', get_login_cookie_hash($user_info['user_name'], $user_info['password'], $user_info['salt'], $user_info['uid'], false));
				
				$this->model('openid_bistu')->update_token($bistu_user['userid'], AWS_APP::session()->bistu_token);
			...
			else
			{
				if ($this->user_id)
				{
					$this->model('openid_bistu')->bind_account(AWS_APP::session()->bistu_profile, '/', $this->user_id);
				}
				else
				{
					if (get_setting('register_type') == 'close')
					...
					else
					{
						...
						TPL::assign('user_name', AWS_APP::session()->bistu_profile['userid']);
